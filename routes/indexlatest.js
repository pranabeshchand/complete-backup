    ...
    co(function* () {
        try {
            ...
            if (task == 'import') {
                ...
                yield processCSV(mappingobj, sfconnection, userId);
...
function processCSV(mappingobj, connection, userId) {
    var defer = q.defer();
    var jsondata = [];
    var relatedJsonData = [];
        ...
        .on('end', function (data) {
            ...
            co(function* () {
                try {
                        ...
                        while (currentIndex < dataArray.length) {
                            ...
                            try {
                                if (validRecords.length > 0) {
                                    ...
                                    var relatedData = yield insertIntoRelatedObjects(relatedObjName, validRecords, connection);
                                    console.log("relatedData is " + JSON.stringify(relatedData, null, 2));
                                    ...
                                    for (var i = 0; i < relatedData.length; i++) {
                                        for (var k = 0; k < validRecordsIndexes.length; k++) {
                                            ...
                                            data[relation] = relatedData[i].id
                                        }
                                    }
                    ...
                    console.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
                    currentIndex = 0;
                    fields = yield salesObjectAttributes(userId, targetSalesObject);
                    ...
                    for(var i=0; i<salesdata.length; i++) {
                        ...
                        var result = validateRecord(record,fields,requiredSalesAttributes);
                        if (result.valid) {
                            validRecords.push({index: k, records: result.record});
                            validRecordsIndexes.push(k);
                        } else {
                    ...
                    dataArray = salesdata;
                    console.log("dataArray is " + JSON.stringify(dataArray,null,2));
                    var successRecords = [];
                    var errorRecords = [];
                    while (currentIndex < dataArray.length) {
                        console.log("current index is " + currentIndex);
                        ...
                        try {
                            if (validRecords.length > 0) {
                                // console.log("valid records for insertion " + JSON.stringify(validRecords, null, 2))
                                var insertedRecords = yield insertIntoRelatedObjects(targetSalesObject, validRecords, connection);
